...
echo
echo "Usage: integration-test.sh <-n> <-c> <-s> <-m>"
...
echo "        -m is good for CI type automated tests. This is only done with Harness shut down so"
echo "        this option is ignored if -s is skipping restarts."
echo
echo "        NOTE: This test assumes Harness is running"
echo
