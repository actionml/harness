...
class EngineServiceImpl(implicit inj: Injector) extends EngineService{
  ...
  override def receive: Receive = {
    ...
    case UpdateEngineWithTrain(engineJson) =>
      log.info(s"Update existing engine, ${engineJson}")
      sender() ! admin.updateEngineWithTrain(engineJson).map(_.asJson)
...
case class UpdateEngineWithTrain(engineJson: String) extends EngineAction
